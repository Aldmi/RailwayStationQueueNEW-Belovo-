...
namespace Server.Entitys
{
    ...
    public class TicketItem
    {
        ...
        public override string ToString()
        {
            ...
            return $" Дата добавления в очередь: {AddedTime}      Дата поступления в обработку: {DateTime.Now}      Номер билета: {ticketName}      Номер кассира:   {  Cashbox?.ToString() ?? "неизвестный кассир" } ";
        }
    }
}